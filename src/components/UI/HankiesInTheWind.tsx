...
const HankiesInTheWind: React.FC<HankiesInTheWindProps> = ({ initialZoom = 6 }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      mainGroup.children.forEach((child: THREE.Object3D) => {
        if (child instanceof THREE.Group) {
          child.children.forEach((line: THREE.Object3D) => {
            // Type casting for THREE.js objects that may have geometry/material
            const lineWithGeometry = line as unknown as { geometry?: { dispose: () => void } }
            const lineWithMaterial = line as unknown as { material?: { dispose: () => void } }
            
            if (lineWithGeometry.geometry) lineWithGeometry.geometry.dispose()
            if (lineWithMaterial.material) lineWithMaterial.material.dispose()
          })
      ...
      if (scene) {
        scene.traverse((object: THREE.Object3D) => {
          if (object instanceof THREE.Mesh) {
            ...
            // Handle material disposal for Line objects
            const lineWithMaterial = object as unknown as { material?: { dispose: () => void } }
            if (lineWithMaterial.material) lineWithMaterial.material.dispose()
          }
        })
      }
